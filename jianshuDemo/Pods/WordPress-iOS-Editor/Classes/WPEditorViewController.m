...
//    _toolbarView.items = [self setAllItemsForToolbar];
}
...
- (NSMutableArray *)itemsForToolbar
{
    ...
    if (items.count>0) {
        [items insertObject:[self hideKeyBoardBarButton] atIndex:0];
    }
	return items;
}

- (NSMutableArray *)setAllItemsForToolbar {
    NSMutableArray *items = [[NSMutableArray alloc] init];
    
    [items addObject:[self hideKeyBoardBarButton]];
...
-(void)didTapHideKeyBoardbarButton {
    [self.view endEditing:YES];
}

- (ZSSBarButtonItem*)hideKeyBoardBarButton
{
    
    ZSSBarButtonItem *barButtonItem = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon_format_keyboard.png"] style:UIBarButtonItemStylePlain target:self action:@selector(didTapHideKeyBoardbarButton)];
    barButtonItem.title = @"hideKeyBoardBarButton";
